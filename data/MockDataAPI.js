...
import { recipes, categories, ingredients,mydata } from './dataArrays';
// import { mydata } from './breakfastdata'; 
...
export function getRecipesByCategoryID(id) {
  // const nameUpper = categoryName.toUpperCase();
  const recipesArray = [];
  categories.map(data => {
    if (data.id===id) {
...
export function getRecipesByRecipeNameAndCateg(recipeName,categoryId) {
  const nameUpper = recipeName.toUpperCase();
...
recipes.map(data => {
    if ( data.categoryId===categoryId &&  data.title.toUpperCase().includes(nameUpper)) {
      recipesArray.push(data);
    }
